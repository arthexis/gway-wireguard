...
`status` reports the live `gway` WireGuard interface without exposing private key material. `peer managed` reads the persistent WireGuard configuration and lists only peer blocks explicitly owned by gway-wireguard. The command surface is generated by GWAY from the public functions under `gway_wireguard.gway`; this repository does not maintain a second handwritten parser for those commands.

The existing enrollment and administrative scripts remain available while that operational surface is migrated deliberately.
