...
Phase 3 removes the manual server-side peer-registration step from a fresh client install. The gateway now owns device registration, one-time enrollment tokens, VPN address allocation, managed WireGuard peers, and revocation.
...
- `server/admin.py` — registry initialization, token creation, device listing, and revocation.
...
The default registry is `/var/lib/gway-wireguard/registry.sqlite3`. Enrollment tokens are stored only as SHA-256 hashes. A token is consumed only after the peer update succeeds.
...
Revocation removes the live WireGuard peer, removes its managed persistent block, and marks the registry record disabled. A revoked device cannot re-enroll merely by obtaining another token.
...
```bash
ping -c 3 <assigned-device-vpn-ip>
ssh arthe@<assigned-device-vpn-ip>
```

Phase 3 is live-validated when a fresh box enrolls with one token, the gateway adds it without manual WireGuard editing or disturbing `gway-001`, bidirectional tunnel access works, replaying the consumed token fails, and server-side revocation removes access.
