...
def test_peer_failure_reports_stage_without_logging_token() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        with patch(
            "enroll_api.new_request_id",
            return_value="req-stage",
        ):
            with patch.object(
                service.peers,
                "ensure_peer",
                ...
                with redirect_stderr(stderr):
                    try:
                        ...
                        raise AssertionError("expected PeerManagerError")

        events = [json.loads(line) for line in stderr.getvalue().splitlines()]
        failure = next(event for event in events if event["event"] == "enrollment_failed")
        assert failure["request_id"] == "req-stage"
        assert failure["device_id"] == "gway-004"
        assert failure["stage"] == "peer_apply"
        assert failure["exception"] == "PeerManagerError"
